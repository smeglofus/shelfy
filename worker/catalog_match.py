...
def title_lookup_result_is_trustworthy(
    ...
    if query_author and result_author:
        if authors_match(query_author, result_author):
            ...
            # the title, either as a close ratio ("Válka s mloky" vs "Válka s
            # mloky (2. vyd.)") or as full word containment ("Příběh lásky" ⊂
            # "Příběh lásky: jak a proč milujeme", whose ratio dips below the
            # bar purely from the subtitle's length).
            return title_score >= SUGGEST_THRESHOLD or _query_title_contained(query_title, result_title)
