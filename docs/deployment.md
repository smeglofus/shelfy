...
# Frontend
docker build -t "$REGISTRY/shelfy-frontend:$TAG" frontend
# Worker
...
Create local files first (recommended: under a temporary `./secrets/` directory):
...
openssl rand -base64 64 > secrets/jwt_secret_key.txt
printf 'admin@example.com' > secrets/admin_email.txt
printf 'change-this-admin-password' > secrets/admin_password.txt
```
...
docker secret create admin_email secrets/admin_email.txt
docker secret create admin_password secrets/admin_password.txt
```

> Important: if you need to rotate a secret value, remove and recreate the secret, then redeploy the stack.
